...
public:
  ...
  void enforce(Node& node, float dt) {
    float x = (node.position[0] - basePoint[0]) * normal[1]
            - (node.position[1] - basePoint[1]) * normal[0];
    if ((x > 0) && (x < width)) {
  ...
  float basePoint[2];      // left-most surface reference position
  float normal[2];         // surface normal
  float width;             // surface width measured relative (to the right) to the base point
